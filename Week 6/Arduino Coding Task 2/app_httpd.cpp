...
static void draw_face_boxes(fb_data_t *fb, std::list<dl::detect::result_t> *results, int face_id)
{
    ...
    for (std::list<dl::detect::result_t>::iterator prediction = results->begin(); prediction != results->end(); prediction++, i++)
    {
        ...
        if (i == 0) { // Follow 1st face detected
            int centerX = x + (w / 2);
            ...
            // Serial.printf("Face Center X: %d, Servo Angle: %d\n", centerX, angle);
        }
        // ---------------------------------
...
/*    auto face = results.front();
    ...
    int face_center = (x1 + x2) / 2;
    int screen_center = 160;

    int error = face_center - screen_center;
...
static esp_err_t index_handler(httpd_req_t *req){
    // 1. Beritahu browser ini adalah fail HTML biasa (bukan gzip)
    httpd_resp_set_type(req, "text/html");
    
    // 2. Ambil sensor untuk pastikan kamera ada (optional tapi baik untuk safety)
    sensor_t *s = esp_camera_sensor_get();
    if (s == NULL) {
    ...
    // 3. Hantar fail index_ov2640_html_gz yang kita dah edit jadi HTML biasa tadi
    // Kita guna strlen() sebab kita tak guna length fixed (0) tadi
